...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    #    outputs
    # Fully-connected model with 2 hidden layers
    model_transfer.fc  = nn.Sequential(
        nn.Linear(num_ftrs, 5 * n_classes),
        nn.BatchNorm1d(5 * n_classes),
        nn.ReLU(),
        nn.Dropout(p=0.1),
        nn.Linear(5 * n_classes, 3 * n_classes),
        nn.BatchNorm1d(3 * n_classes),
        nn.ReLU(),
        nn.Dropout(p=0.1),
